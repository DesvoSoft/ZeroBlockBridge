...
- [x] Research Playit API: auth flow (setup code → secret key), tunnel CRUD, agent registration
- [x] Create `app/services/playit_api.py` with PlayitApiClient class:
  - `link_account(setup_code)` — (Opted for Direct Claim via CLI output)
...
- [x] Implement tunnel cache (JSON file, resilient to API failures) - handled via direct polling
- [x] Implement per-server tunnel claiming (`tunnel.in_use` flag)
- [x] Integrate with existing `PlayitManager`: use `PlayitApiClient` for tunnel operations, keep agent lifecycle
- [x] Add agent key input and link flow to tunnel setup UI - (Skipped, using Direct Claim UI)
