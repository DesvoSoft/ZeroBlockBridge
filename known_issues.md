...
Needs field re-test with a stale secret to confirm the escalation UX. Bug 1 (WinError 216) still open: needs repro (check `PLAYIT_URL_WINDOWS` binary matches host arch, verify download integrity — download to temp + size check + `playit version` smoke test + atomic rename, same class as JDK checksum verification in `java_installer.py`). ARM64 Windows currently enters infinite redownload cycle (version check fails → redownload same x86_64 binary).
